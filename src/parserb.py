...
# Change the parse_ methods and try to put them into a class rather than be just methods.

class ParserbError(Exception):
	pass
...
def parse_verb(word_list):
	...
	if peek(word_list) == 'verb':
		return match(word_list, 'verb')
		 ...
		 raise ParserbError("Expected a verb next.")
...
def parse_object(word_list):
	...
	if next == 'noun':
		...
		raise ParserbError("Expected a noun or direction next.")
...
def parse_sentence(word_list):
	...
	if start == 'noun':
		...
		raise ParserbError("Must start with subject, object or verb not: %s" % start)
